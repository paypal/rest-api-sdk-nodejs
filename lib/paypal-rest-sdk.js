...
module.exports = function () {
    ...
    function invoke(http_scheme, http_method, path, data, http_options_param, cb) {
        ...
        req.on('response', function (res) {
            ...
            function () {
                ...
                var httpStatusCode = res.statusCode;
                ...
                if (!err) {
                    if (httpStatusCode < 200 || httpStatusCode >= 300) {
                        err = new Error('Response Status : ' + httpStatusCode);
                        err.response = response;
                        err.httpStatusCode = httpStatusCode;
                        response = null;
                    }
    ...
    function openid_connect_request(path, data, config, cb) {
        ...
        invoke('https', 'POST', path, querystring.stringify(data), http_options, cb);
    }
    ...
    return {
        ...
        payment: {
            create: function (data, config, cb) {
                executeHttp('https', 'POST', '/v1/payments/payment/', data, config, cb);
            },
            get: function (payment_id, config, cb) {
                executeHttp('https', 'GET', '/v1/payments/payment/' + payment_id, {}, config, cb);
            },
            list: function (data, config, cb) {
                executeHttp('https', 'GET', '/v1/payments/payment', data, config, cb);
            },
            execute: function (payment_id, data, config, cb) {
                executeHttp('https', 'POST', '/v1/payments/payment/' + payment_id + '/execute', data, config, cb);
            }
        },
        sale: {
            refund: function (sale_id, data, config, cb) {
                executeHttp('https', 'POST', '/v1/payments/sale/' + sale_id + '/refund', data, config, cb);
            },
            get: function (sale_id, config, cb) {
                executeHttp('https', 'GET', '/v1/payments/sale/' + sale_id, {}, config, cb);
            }
        },
        refund: {
            get: function (refund_id, config, cb) {
                executeHttp('https', 'GET', '/v1/payments/refund/' + refund_id, {}, config, cb);
            }
        },
        authorization: {
            get: function (authorization_id, config, cb) {
                executeHttp('https', 'GET', '/v1/payments/authorization/' + authorization_id, {}, config, cb);
            },
            capture: function (authorization_id, data, config, cb) {
                executeHttp('https', 'POST', '/v1/payments/authorization/' + authorization_id + '/capture', data, config, cb);
            },
            void: function (authorization_id, config, cb) {
                executeHttp('https', 'POST', '/v1/payments/authorization/' + authorization_id + '/void', {}, config, cb);
            },
            reauthorize: function (authorization_id, data, config, cb) {
                executeHttp('https', 'POST', '/v1/payments/authorization/' + authorization_id + '/reauthorize', data, config, cb);
            }
        },
        capture: {
            refund: function (capture_id, data, config, cb) {
                executeHttp('https', 'POST', '/v1/payments/capture/' + capture_id + '/refund', data, config, cb);
            },
            get: function (capture_id, config, cb) {
                executeHttp('https', 'GET', '/v1/payments/capture/' + capture_id, {}, config, cb);
            }
        },
        credit_card: {
            create: function (data, config, cb) {
                executeHttp('https', 'POST', '/v1/vault/credit-card/', data, config, cb);
            },
            get: function (credit_card_id, config, cb) {
                executeHttp('https', 'GET', '/v1/vault/credit-card/' + credit_card_id, {}, config, cb);
            },
            delete: function (credit_card_id, config, cb) {
                executeHttp('https', 'DELETE', '/v1/vault/credit-card/' + credit_card_id, {}, config, cb);
            }
        },
        openid_connect: {
            tokeninfo: {
