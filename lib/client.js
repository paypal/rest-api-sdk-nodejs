...
var invoke = exports.invoke = function invoke(http_method, path, data, http_options_param, cb) {
    ...
    request(http_options, request_data, cb, retries);
};

function request(http_options, data, cb, retries) {
    ...
    req.on('timeout', function () {
        if (retries > 0) {
            req.abort();
            retries--;
            request(http_options, data, cb, retries);
        }
    });
    ...
    req.on('response', function (res) {
        ...
        res.on('end', function () {
            ...
            if (!err && res.statusCode === 500 && retries > 0) {
                retries--;
                req.abort();
                return request(http_options, data, cb, retries);
            }
