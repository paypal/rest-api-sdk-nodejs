 ...
 */
const invoke = exports.invoke = function invoke(http_method, path, data, http_options_param, cb) {
    const client = (http_options_param.schema === 'http') ? http : https;
    ...
    const req = client.request(http_options);
    req.on('error', e => {
    ...
    req.on('response', res => {
        ...
        res.on('end', () => {
            ...
            try {
                ...
                err = new Error(`Invalid JSON Response Received. If the response received is empty, please checkthe httpStatusCode attribute of error message for 401 or 403. It is possible that the client credentialsare invalid for the environment you are using, be it live or sandbox.`);
                err.error = {
