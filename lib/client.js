...
var invoke = exports.invoke = function invoke(http_method, path, data, http_options_param, cb) {
    ...
    //HTTPS Proxy
    if (configuration.default_options.proxy_host !== '' && configuration.default_options.proxy_host !== '') {
        http_options.agent = new HttpsProxyAgent('http://' + configuration.default_options.proxy_host + configuration.default_options.proxy_port);
        // explicitly set this since it will default to 80 instead of 443!
        // this is because the proxy is http (80), and not https (443)
