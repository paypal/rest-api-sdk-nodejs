...
var configure = exports.configure = function configure(options) {
    if (typeof options === 'object') {
        ...
        if (options.mode) {
            var defaultApiEndpoint = utils.getDefaultApiEndpoint(options.mode);
            var defaultEndpoint = utils.getDefaultEndpoint(options.mode);

            configuration.default_options.host = options.host || defaultApiEndpoint;
            configuration.default_options.openid_connect_host = options.openid_connect_host || defaultApiEndpoint;
            configuration.default_options.logout_url = options.logout_url || 'https://www.' + defaultEndpoint + '/webapps/auth/protocol/openidconnect/v1/endsession';
            configuration.default_options.authorize_url = options.authorize_url || 'https://www.' + defaultEndpoint + '/signin/authorize';
        }
    }
...
var executeHttp = exports.executeHttp = function executeHttp(http_method, path, data, http_options, cb) {
    ...
    http_options.host = http_options.host || configuration.default_options.host;
