 ...
 */
const generateToken = exports.generateToken = function generateToken(config, cb) {
    ...
    client.invoke('POST', '/v1/oauth2/token', payload, http_options, (err, res) => {
        let token = null;
        if (res) {
...
function updateToken(http_options, error_callback, callback) {
    generateToken(http_options, (error, token) => {
        if (error) {
 ...
 * @param  {Function} cb                    Callback function
 */
const executeHttp = exports.executeHttp = function executeHttp(http_method, path, data, http_options, cb) {
    ...
    if (http_options.client_id in token_persist && !utils.checkExpiredToken(token_persist[http_options.client_id]) && !http_options.refresh_token) {
        ...
        client.invoke(http_method, path, data, http_options, (error, response) => {
