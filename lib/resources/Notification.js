 ...
 */
function webhook() {
    const baseURL = '/v1/notifications/webhooks/';
    const operations = ['create', 'list', 'get', 'del', 'delete'];
...
function webhookEvent() {
    const baseURL = '/v1/notifications/webhooks-events/';
    ...
    function getAndVerify(body, cb) {
        ...
        let err = null;
        try {
            const webhookEventId = JSON.parse(body).id;
            api.executeHttp('GET', baseURL + webhookEventId, {}, (error, res) => {
                if (error) {
                    cb(error, response);
                } else {
                    cb(err, true);
...
function webhookEventType() {
    const baseURL = '/v1/notifications/webhooks-event-types/';
