 ...
 */
function authorizeUrl(data={}, config=configuration.default_options) {
    //Use mode provided, live or sandbox to construct authorize_url, sandbox is default
    const url = `https://www.${utils.getDefaultEndpoint(config.mode)}/signin/authorize` || config.authorize_url;
 ...
 */
function logoutUrl(data={}, config=configuration.default_options) {
    const url = `https://www.${utils.getDefaultEndpoint(config.mode)}/webapps/auth/protocol/openidconnect/v1/endsession` || config.logout_url;
