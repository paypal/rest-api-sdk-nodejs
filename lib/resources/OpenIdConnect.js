...
function openIdConnectRequest(path, data, config, cb) {
    var http_options = {
        ...
        host: config.openid_connect_host || configuration.default_options.openid_connect_host,
...
function authorizeUrl(data, config) {
    ...
    var url = config.authorize_url || 'https://www.' + utils.getDefaultEndpoint(config.mode) + '/signin/authorize';
...
function logoutUrl(data, config) {
    ...
    var url = config.logout_url || 'https://www.' + utils.getDefaultEndpoint(config.mode) + '/webapps/auth/protocol/openidconnect/v1/endsession';
